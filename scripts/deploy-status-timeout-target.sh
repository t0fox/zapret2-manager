...
# Direct SCP-equivalent deployment for the canonical status timeout fix.
# This is intentionally backend-only: no APK build/install and no UI assets.
ROOT=${ROOT:-$(CDPATH= cd -- "${0%/*}/.." && pwd)}
TARGET=${TARGET:-root@192.168.1.1}
REMOTE_ROOT=${REMOTE_ROOT:-/tmp/z2m-status-timeout}
BACKUP_ROOT=${BACKUP_ROOT:-$REMOTE_ROOT/backup}
SSH_CONNECT_TIMEOUT=${SSH_CONNECT_TIMEOUT:-5}
EXPECTED_COMMIT=${EXPECTED_COMMIT:-}

[ "${CONFIRM_TARGET_DEPLOY:-}" = STATUS_ONLY ] || {
    printf '%s\n' 'Refusing target mutation. Set CONFIRM_TARGET_DEPLOY=STATUS_ONLY.' >&2
    exit 2
}
[ -n "$EXPECTED_COMMIT" ] || {
    printf '%s\n' 'Refusing target mutation. Set EXPECTED_COMMIT to the committed status-fix HEAD.' >&2
    exit 2
}
if [ -f "$ROOT/.git" ]; then
...
zapret2-manager/files/usr/libexec/zapret2-manager/strategy-status.uc|/usr/libexec/zapret2-manager/strategy-status.uc
'
...
printf 'Deployed canonical status timeout fix at %s; backup: %s\n' "$EXPECTED_COMMIT" "$BACKUP_ROOT"
