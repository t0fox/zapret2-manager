#!/bin/sh
# scripts/test/run-all-tests.sh — run every repository self-test suite.
...
while IFS= read -r f; do
	...
	case "$f" in
		*.test.mjs)
			out="$("$NODE" --test "$f" 2>&1)"
			rc=$?
			p="$(printf '%s\n' "$out" | sed -n -e 's/.*ℹ pass \([0-9][0-9]*\).*/\1/p' -e 's/^[[:space:]]*# pass \([0-9][0-9]*\).*/\1/p' | tail -1)"
			fa="$(printf '%s\n' "$out" | sed -n -e 's/.*ℹ fail \([0-9][0-9]*\).*/\1/p' -e 's/^[[:space:]]*# fail \([0-9][0-9]*\).*/\1/p' | tail -1)"
			note=""
			if [ "$rc" -ne 0 ]; then
