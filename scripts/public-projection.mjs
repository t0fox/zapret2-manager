...
const CODE_EVIDENCE = {
  ...
  'docs/03-products/z2k-core.md': [{ path: 'zapret2-manager/files/usr/libexec/zapret2-manager/resource-update.uc', start: 3141, end: 3185, symbol: 'resource_center_update', why: 'Подготовленный Z2K target применяется через текущую bounded transaction Resource Center.' }],
  ...
  'docs/03-products/asset-registry.md': [{ path: 'zapret2-manager/files/usr/libexec/zapret2-manager/asset-registry.uc', start: 260, end: 265, symbol: 'asset_registry_import', why: 'Typed asset import нормализует content, provenance и SHA-256 перед публикацией metadata.' }],
  'docs/03-products/nfqueue.md': [{ path: 'zapret2-manager/files/usr/libexec/zapret2-manager/core/status-collector.uc', start: 165, end: 192, symbol: 'health_block', why: 'Engine status read-back публикует NFQUEUE number, registration, owner evidence и queue health без второго Scanner owner.' }],
  'docs/03-products/dnsmasq.md': [{ path: 'zapret2-manager/files/usr/libexec/zapret2-manager/dns-product.uc', start: 65, end: 95, symbol: 'dns_product_validate', why: 'DNS facade публикует текущую проверку, а writers сохраняют владение мутациями по scope.' }],
  ...
  'docs/08-development/z2k-avatar-integration.md': [{ path: 'zapret2-manager/files/usr/libexec/zapret2-manager/strategy-source-z2k.uc', start: 330, end: 356, symbol: 'strategy_source_z2k_info', why: 'Z2K source adapter фиксирует отдельный source snapshot lifecycle; Avatar остаётся независимым strategy source.' }],
  ...
  'docs/08-development/apk-build.md': [{ path: '.github/workflows/apk-build.yml', start: 1, end: 44, symbol: 'OpenWrt full APK build', why: 'Единственный автоматизированный workflow собирает и проверяет один self-contained manager APK, затем публикует rolling main-latest release files.' }],
