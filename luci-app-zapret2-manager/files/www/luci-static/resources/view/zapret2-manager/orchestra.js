...
const probePreflightRpc = rpc.declare({ object: 'zapret2-manager', method: 'orchestra_probe_preflight', reject: true });
...
function isDomNode(value) { return !!(value && typeof value === 'object' && (value.nodeType === 1 || typeof value.appendChild === 'function' || value.tagName)); }
function nodeValue(value) { if (isDomNode(value)) return value; if (Array.isArray(value)) return esc(value.map(function (item) { return isDomNode(item) ? '' : item == null ? '' : String(item); }).join(', ')); return esc(value); }
function kv(label, value) { return E('div', { 'class': 'z2m-kv' }, [E('span', { 'class': 'z2m-kv-label' }, esc(label)), E('span', { 'class': 'z2m-kv-value' }, nodeValue(value))]); }
...
var CATEGORY_LABELS = { ai: 'Искусственный интеллект', AI: 'Искусственный интеллект', social: 'Социальные сети', music: 'Музыка', video: 'Видео', development: 'Разработка', media: 'Медиа', games: 'Игры', gaming: 'Игры', messaging: 'Мессенджеры', other: 'Другое', others: 'Другое' };
...
function overview_runtime(runtime, available) {
	...
	var nfqws2 = !available || process.found == null ? { label: _('Состояние не подтверждено'), displayLabel: _('Состояние неизвестно'), status: 'unknown' } : process.found === false ? { label: _('Не работает'), displayLabel: _('Остановлен'), status: 'failed' } : runtime.status === 'starting' || process.identityVerified !== true ? { label: _('Запускается'), displayLabel: _('Запускается'), status: 'running' } : { label: _('Работает'), displayLabel: _('Работает'), status: 'healthy' };
	var nfqueue = !available || queue.registered == null ? { label: _('Состояние неизвестно'), status: 'unknown' } : queue.registered === false ? { label: _('Не подключена'), status: 'failed' } : queue.ownerMatches !== true ? { label: _('Подтверждена частично'), status: 'partial' } : { label: _('Подключена'), status: 'healthy' };
	var configuration = !available || current.appliedMatch == null ? { label: _('Не проверено'), displayLabel: _('Не проверено'), status: 'unknown' } : current.appliedMatch === false ? { label: _('Обнаружено расхождение'), displayLabel: _('Отличается'), status: 'divergent' } : { label: _('Runtime совпадает с сохранённой'), displayLabel: _('Совпадает'), status: 'healthy' };
	var verification = !available || current.verification == null || current.verification === 'unknown' ? { label: _('Не выполнялась'), status: 'unknown' } : current.verification === 'verified' ? { label: _('Подтверждена'), status: 'verified' } : { label: _('Подтверждена частично'), status: 'partial' };
...
function overview_model(state) {
	...
	var autoTruth = Z2M.ui.activeRunTruth(auto), stateTruth = Z2M.ui.activeRunTruth({ phase: state.activeRun && state.activeRun.phase, activeRun: state.activeRun });
	var activeTruth = autoTruth.active ? autoTruth : stateTruth.active ? stateTruth : { active: false, stale: autoTruth.stale || stateTruth.stale, reasonCode: autoTruth.stale ? autoTruth.reasonCode : stateTruth.reasonCode, run: null };
	var active = activeTruth.active ? activeTruth.run : null;
	...
	else if (runtime.status === 'running' && runtime.nfqws2.status === 'healthy' && runtime.nfqueue.status === 'healthy' && runtime.verification.status === 'verified' && runtime.configuration.status === 'healthy') overall = { status: 'healthy', label: _('Работает') };
	else if (runtime.status === 'running' && runtime.nfqws2.status === 'healthy' && runtime.nfqueue.status === 'healthy') overall = { status: 'degraded', label: _('Работает; проверка конфигурации не завершена') };
	...
	if (activeTruth.stale) warnings.push({ title: _('Предыдущая проверка не была корректно завершена'), message: _('Сохранённое состояние запуска устарело; обновите статус перед новой проверкой.'), panel: 'orchestra-auto', reasonCode: 'stale-run' });
	...
	else if (active) primary = { kind: 'open-run', label: _('Открыть текущий подбор'), panel: 'orchestra-auto' };
	else if (auto && auto.enabled === false && (admission.enable || {}).allowed === true) primary = { kind: 'enable-auto', label: _('Включить автоподбор'), panel: 'orchestra-auto' };
	else if (auto && (admission.runNow || {}).allowed === true) primary = { kind: 'run-now', label: _('Проверить сейчас'), panel: 'orchestra-auto' };
...
return L.view.extend({
	...
	load: function () {
		...
		var calls = [
			...
			['catalogStatus', function () { return get(catalogStatusRpc); }], ['health', function () { return get(healthGetRpc); }], ['readiness', function () { return get(probePreflightRpc); }]
		];
		if (this._panel === 'orchestra-overview') calls.unshift(['managerStatus', function () { return get(managerStatusRpc); }]);
		if (this._panel === 'orchestra-auto' || this._panel === 'orchestra-adaptive') calls.push(['caps', function () { return get(capsRpc); }], ['legacyEvents', function () { return get(legacyEventsRpc); }], ['legacyHistory', function () { return get(legacyHistoryRpc); }], ['legacyRatings', function () { return get(legacyRatingsRpc); }]);
		...
		return loadWave(0, {}).then(function (a) {
			self._state.caps = a.caps && !a.caps._error ? a.caps : null; self._state.readiness = a.readiness && !a.readiness._error ? a.readiness : null; self._state.adaptive = a.adaptive._error ? null : a.adaptive;
	...
	_panelFromHash: function () {
		...
		return entry ? (entry.key === 'auto' ? entry.route : entry.legacyRoute || entry.route) : 'orchestra-overview';
	},
	...
	_setPanel: function (panel) {
		...
		panel = entry.key === 'auto' ? entry.route : entry.legacyRoute || entry.route;
	...
	render: function (state) {
		...
		var self = this, content = E('div', { 'class': 'z2m-orchestra-content' }), overview = this._overviewModel(this._state), autoHeader = this._panel === 'orchestra-auto' ? this._autoHeader() : null;
		var root = Z2M.ui.PageShell({
			id: 'z2m-orchestra-page', className: 'z2m-orchestra-shell z2m-orchestra',
			header: this._panel === 'orchestra-overview' ? this._overviewHeader(overview) : autoHeader || Z2M.ui.PageHeader({ title: _('Orchestra'), description: _('Find, compare and safely apply a verified strategy.') }),
	...
	_renderContent: function (content) {
		...
		else if (this._panel === 'orchestra-auto') content.appendChild(this._autoStrategySection());
	...
	_overviewRow: function (label, item) {
		...
		return E('div', { 'class': 'z2m-overview-row' }, [E('span', { 'class': 'z2m-overview-row-label' }, label), Z2M.ui.StatusBadge({ status: item.status, label: item.displayLabel || item.label })]);
	},
	...
	_overviewSection: function () {
		...
		body.appendChild(E('div', { 'class': 'z2m-overview-grid' }, [
			...
			this._overviewSummary(_('Автоматический подбор'), !model.auto ? [E('span', {}, _('Статус автоподбора недоступен.'))] : [this._overviewRow(_('Состояние'), { label: model.auto.enabled ? _('Включён') : _('Отключён'), status: model.auto.enabled ? 'healthy' : 'disabled' }), this._overviewRow(_('Фаза'), { label: model.auto.phase, status: model.operation ? 'running' : model.auto.enabled ? 'healthy' : 'disabled' }), E('div', { 'class': 'z2m-overview-row' }, [E('span', { 'class': 'z2m-overview-row-label' }, _('Последнее обновление состояния')), E('span', {}, timestamp_label(model.auto.lastCheckAt, 'Время не указано'))]), E('div', { 'class': 'z2m-overview-row' }, [E('span', { 'class': 'z2m-overview-row-label' }, _('Последний запуск проверки')), E('span', {}, model.timestamps.lastRunLabel)]), model.auto.cooldownUntil ? E('div', { 'class': 'z2m-overview-row' }, [E('span', { 'class': 'z2m-overview-row-label' }, _('Повторная проверка')), E('span', {}, _('Повторная проверка отложена'))]) : E('span', {}), this._overviewRow(_('Последняя рабочая стратегия'), { label: model.auto.lastGood ? _('Доступна') : _('Последняя рабочая стратегия отсутствует'), status: model.auto.lastGood ? 'verified' : 'unknown' })]),
	...
	_servicesSection: function () {
		...
		if (discord) {
			var discordReadiness = s.readiness || {}, readinessOk = discordReadiness.ok === true, readinessText = readinessOk ? _('Готово: transport, scanner, curl и service catalog доступны.') : (discordReadiness.reasonCode || _('Probe dependencies unavailable'));
			var discordButton = btn(_('Find Discord strategies'), function (b) {
				self._busy(b, _('Starting…'));
				rpcCall(runStartRpc, pack({ targetType: 'service', targetId: discord.id, candidateMode: 'zapret2gui-only', repeats: 1, maxCandidates: 4, maxAttempts: 12, perAttemptTimeoutSec: 10, totalTimeoutSec: 180 })).then(function (x) {
			...
			}, !readinessOk || !!(self._state.activeRun && !terminalRun(self._state.activeRun.phase, (self._state.caps || {}).terminalPhases)), 'cbi-button-action', !readinessOk ? readinessText : null);
			body.appendChild(E('div', { 'class': 'z2m-card z2m-discord-service-card' }, [E('h4', {}, _('Discord TCP/443')), E('p', {}, _('Dynamically test Web, Gateway and CDN against the packaged Zapret2GUI registry.')), kv(_('Probe readiness'), badge(readinessText, readinessOk ? 'ok' : 'bad')), E('div', { 'class': 'z2m-actions' }, [discordButton])]));
		}
	...
	_runDetail: function (run) {
		...
		body.appendChild(this._candidateJournal(run)); body.appendChild(this._rankingTable(run, protocol));
	...
	_serviceRunDetail: function (run) { var body = E('div', { 'class': 'z2m-orchestra-run-detail' }), verdict = run.serviceVerdict || {}, groups = run.serviceResults || []; body.appendChild(E('div', { 'class': 'z2m-orchestra-detail-top' }, [E('div', {}, [E('h4', {}, esc(run.serviceId || run.target || _('Service'))), E('p', {}, _('Apply is available only for the verified domain workflow.'))]) ])); body.appendChild(E('div', { 'class': 'z2m-orchestra-state-grid' }, [kv(_('Domains'), (verdict.finishedDomains || 0) + ' / ' + (verdict.totalDomains || (run.targets || []).length)), kv(_('Confirmed winners'), verdict.domainsWithConfirmedWinner || 0), kv(_('Without winner'), verdict.domainsWithoutWinner || 0), kv(_('Failed / indeterminate'), (verdict.failedDomains || 0) + ' / ' + (verdict.indeterminateDomains || 0))])); body.appendChild(this._candidateJournal(run)); if (!groups.length) body.appendChild(E('div', { 'class': 'z2m-empty' }, terminalRun(run.phase) ? _('No per-domain result was recorded.') : _('Per-domain results will appear while the service run progresses.'))); groups.forEach(function (group) { var box = E('div', { 'class': 'z2m-card' }, [E('h4', {}, esc(group.domain))]); (group.protocols || []).forEach(function (protocol) { var winner = protocol.winner; box.appendChild(E('div', { 'class': 'z2m-orchestra-result-panel' }, [kv(_('Protocol'), protocol.protocol === 'tcp_https' ? _('HTTPS / TCP') : _('QUIC / UDP')), kv(_('Winner'), winner ? winner.candidateId : _('No confirmed winner')), E('pre', { 'class': 'z2m-mono' }, esc((protocol.rankedResults || []).map(function (r) { return (r.candidateId || '—') + ' · ' + (r.successCount || 0) + ' / ' + (r.attemptCount || 0) + ' · ' + (r.verdict || 'unknown'); }).join('\n') || _('No candidate evidence.')))])); }); body.appendChild(box); }); body.appendChild(details(_('Technical details'), E('pre', { 'class': 'z2m-mono' }, esc(pack({ serviceId: run.serviceId, catalogVersion: run.catalogVersion, catalogDigest: run.catalogDigest, targets: run.targets, serviceVerdict: run.serviceVerdict }))))); return body; },
	_candidateJournal: function (run) {
		var raw = run && (run.candidateJournal || (run.canonical && run.canonical.candidates));
		if (raw == null) return Z2M.ui.EmptyState({ title: _('Проверенные стратегии'), explanation: _('Результаты этого запуска пока недоступны') });
		if (!Array.isArray(raw)) return Z2M.ui.EmptyState({ title: _('Проверенные стратегии'), explanation: _('Результаты этого запуска пока недоступны') });
		if (!raw.length) return Z2M.ui.EmptyState({ title: _('Проверенные стратегии'), explanation: _('Стратегии ещё не проверялись') });
		var rows = raw, counts = { tested: 0, total: rows.length, working: 0, failed: 0, infrastructure: 0, remaining: 0 }, statusLabels = { 'infrastructure-error': _('Ошибка инфраструктуры'), failed: _('Не прошла'), working: _('Работает'), confirmed: _('Подтверждена'), partial: _('Частично работает'), pending: _('Ожидает проверки'), testing: _('Проверяется'), stopped: _('Остановлена'), 'timed-out': _('Не завершена из-за таймаута'), stale: _('Устаревший результат') };
		rows.forEach(function (row) { var status = String(row.status || 'pending'); if (status === 'pending' || status === 'testing') counts.remaining++; else counts.tested++; if (status === 'working' || status === 'confirmed') counts.working++; if (status === 'failed' || status === 'stopped' || status === 'timed-out') counts.failed++; if (status === 'infrastructure-error') counts.infrastructure++; });
		var body = [], caption = _('Проверено ') + counts.tested + ' из ' + counts.total + ' · ' + _('Работают ') + counts.working + ' · ' + _('Не прошли ') + counts.failed + ' · ' + _('Ошибки ') + counts.infrastructure + ' · ' + _('Осталось ') + counts.remaining;
		body.push(E('div', { 'class': 'z2m-orchestra-table-caption' }, esc(caption)));
		function listText(value) { return Array.isArray(value) ? value.map(function (x) { return autoText(x, 64); }).join(', ') : autoText(value, 120); }
		function duration(value) { return value == null ? '—' : Math.round(+value || 0) + ' мс'; }
		function statusKind(status) { return status === 'working' || status === 'confirmed' ? 'ok' : status === 'infrastructure-error' || status === 'failed' ? 'bad' : 'warn'; }
		function candidateDetails(row) {
			var targets = row.targetResults || row.targets || [], attempts = row.attempts || row.attemptResults || [], confirmation = row.confirmation || row.confirmationResult || row.confirmationCount;
			var info = [kv(_('Ранг'), row.rank == null ? '—' : row.rank), kv(_('Результаты целей'), Array.isArray(targets) && targets.length ? listText(targets.map(function (x) { return x.targetId || x.id || x.domain || x.status || '—'; })) : _('Нет данных')), kv(_('Попытки'), Array.isArray(attempts) && attempts.length ? attempts.length : (row.attemptsCompleted == null ? '—' : row.attemptsCompleted + ' / ' + (row.attemptsTotal == null ? row.attemptsCompleted : row.attemptsTotal))), kv(_('Подтверждение'), confirmation == null ? _('Не подтверждена') : listText(confirmation)), kv(_('Причина сбоя'), row.failureReason || row.failureClass || _('Нет')), kv(_('Изменение конфигурации'), row.configurationDelta || row.configDelta || _('Не менялась'))];
			var technical = row.technical || {};
			info.push(details(_('Технические детали (технические сведения)'), E('div', {}, [kv(_('Идентификатор кандидата'), row.candidateId), kv(_('Запуск / поколение'), row.runId + ' / ' + (row.generation == null ? '—' : row.generation)), kv(_('Идентификаторы доказательств'), listText(row.evidenceIds || technical.evidenceIds || [])), kv(_('Хэш'), technical.hash || row.hash || '—')])));
			return details(_('Подробнее о стратегии'), E('div', { 'class': 'z2m-candidate-details' }, info));
		}
		var tableRows = rows.map(function (row) {
			var status = String(row.status || 'pending'), label = row.statusLabel || statusLabels[status] || _('Устаревший результат'), markers = [];
			if (row.applied) markers.push(badge(_('Применена'), 'ok'));
			if (row.lastGood) markers.push(badge(_('Последняя рабочая'), 'ok'));
			return E('tr', {}, [E('td', {}, esc(row.rank == null ? '—' : '#' + row.rank)), E('td', {}, [E('strong', {}, esc(row.displayName || row.candidateId || _('Стратегия без названия'))), row.techniqueLabels && row.techniqueLabels.length ? E('small', { 'class': 'z2m-candidate-techniques' }, esc(listText(row.techniqueLabels))) : E('span', {}), candidateDetails(row)]), E('td', {}, badge(label, statusKind(status))), E('td', {}, esc((row.targetsPassed == null ? '—' : row.targetsPassed) + ' / ' + (row.targetsTotal == null ? '—' : row.targetsTotal))), E('td', {}, esc((row.attemptsCompleted == null ? '—' : row.attemptsCompleted) + ' / ' + (row.attemptsTotal == null ? '—' : row.attemptsTotal))), E('td', {}, esc(duration(row.durationMs))), E('td', {}, markers.length ? markers : E('span', {}, '—'))]);
		});
		body.push(E('div', { 'class': 'z2m-table-wrap' }, [E('table', { 'class': 'table z2m-orchestra-ranking-table' }, [E('thead', {}, E('tr', {}, ['#', 'Стратегия', 'Статус', 'Цели', 'Попытки', 'Длительность', 'Метки'].map(function (h) { return E('th', {}, esc(_(h))); }))), E('tbody', {}, tableRows)])]));
		return E('div', { 'class': 'z2m-card z2m-orchestra-candidate-journal' }, [E('h4', {}, _('Проверенные стратегии')), E('div', {}, body)]);
	},
	...
	_autoRunActive: function (auto) { return !!Z2M.ui.activeRunTruth(auto).active; },
	...
	_autoHeader: function () {
		var self = this, auto = this._state.auto || {}, truth = Z2M.ui.activeRunTruth(auto), admission = auto.admissionReasons || {}, primary = null, overall = this._overviewModel(this._state).overall || {};
		if (truth.active) primary = { label: _('Остановить проверку'), run: function (b) { self._autoStop(b); }, disabled: (auto.capabilities || {}).stop !== true, reason: { reasonCode: (admission.stop || {}).reasonCode || 'operation-active' } };
		else if (truth.stale) primary = { label: _('Обновить состояние'), run: function () { return self._autoRefresh(); } };
		else if (auto.enabled === false && (admission.enable || {}).allowed === true) primary = { label: _('Включить автоподбор'), run: function (b) { self._autoEnable(b); } };
		else if ((admission.runNow || {}).allowed === true) primary = { label: _('Запустить проверку'), run: function (b) { self._autoRun(b); } };
		else if ((admission.runNow || {}).reasonCode) primary = { label: _('Проверить снова'), disabled: true, reason: { reasonCode: admission.runNow.reasonCode } };
		var actions = primary && !this._state.autoReadOnly ? [Z2M.ui.ActionButton({ label: primary.label, kind: 'primary', disabled: primary.disabled, reason: primary.reason, onClick: primary.run })] : [], secondary = [Z2M.ui.ActionButton({ label: this._state.autoLoading ? _('Обновление…') : _('Обновить'), onClick: function () { return self._autoRefresh(); }, disabled: this._state.autoLoading })];
		if (!this._state.autoReadOnly && auto.enabled && (admission.disable || {}).allowed === true) secondary.unshift(Z2M.ui.ActionButton({ label: _('Отключить'), kind: 'danger', onClick: function (b) { self._autoDisable(b); } }));
		if (!this._state.autoReadOnly && auto.lastGood && auto.lastGood.available && (auto.capabilities || {}).restoreLastGood) secondary.unshift(Z2M.ui.ActionButton({ label: _('Восстановить последнюю рабочую'), kind: 'danger', onClick: function (b) { self._autoRestore(b); } }));
		return Z2M.ui.PageHeader({ title: _('Автоматический подбор стратегий'), description: _('Подбирает и проверяет стратегии для выбранных сервисов, сохраняя текущую конфигурацию до подтверждения.'), status: { status: overall.status || (truth.active ? 'running' : auto.enabled ? 'healthy' : 'disabled') }, primaryAction: actions[0], secondaryActions: secondary });
	},
	_autoServiceSelector: function (auto) {
		var self = this, services = this._autoServices(), query = String(this._autoFilter || '').toLowerCase(), groups = {}, list = services.filter(function (service) { return !query || String(service.name || service.id).toLowerCase().indexOf(query) >= 0; }), selected = [];
		this._autoChecks = this._autoChecks || {};
		services.forEach(function (service) { if (self._autoChecks[service.id] == null) self._autoChecks[service.id] = (auto.serviceIds || []).indexOf(service.id) >= 0; if (self._autoChecks[service.id]) selected.push(autoText(service.name || service.id, 64)); });
		list.forEach(function (service) { (groups[category_label(service.category)] = groups[category_label(service.category)] || []).push(service); });
		var search = E('input', { 'class': 'cbi-input-text z2m-auto-service-search', 'type': 'search', 'placeholder': _('Поиск сервиса'), 'aria-label': _('Поиск сервиса'), 'value': this._autoFilter || '' });
		search.addEventListener('input', function () { self._autoFilter = search.value || ''; self._refresh(); });
		var body = E('div', { 'class': 'z2m-auto-service-selector' }, [E('div', { 'class': 'z2m-auto-selected-services' }, [E('strong', {}, _('Выбрано: ')), E('span', {}, selected.length ? selected.join(', ') : _('ничего'))]), search]);
		Object.keys(groups).forEach(function (category) { var grid = E('div', { 'class': 'z2m-auto-service-category' }, [E('h4', {}, esc(category))]); groups[category].forEach(function (service) { var input = E('input', { 'type': 'checkbox', 'id': 'z2m-auto-service-' + service.id }); input.checked = !!self._autoChecks[service.id]; input.disabled = !!self._state.autoReadOnly; input.addEventListener('change', function () { self._autoChecks[service.id] = !!input.checked; }); grid.appendChild(E('label', {}, [input, ' ' + esc(autoText(service.name || service.id, 80))])); }); body.appendChild(grid); });
		if (!list.length) body.appendChild(Z2M.ui.EmptyState({ title: _('Сервисы не найдены'), explanation: services.length ? _('Измените фильтр или обновите каталог.') : _('Каталог сервисов пока недоступен.') }));
	...
	_testedStrategies: function (run) {
		...
		return E('section', { 'class': 'z2m-auto-tested-strategies' }, [Z2M.ui.SectionHeader({ title: _('Проверенные стратегии'), description: _('Журнал текущего запуска; порядок и статусы пришли от backend.') }), this._candidateJournal(run)]);
	},
	_autoResultState: function (run) {
		if (!run) return E('span', {}); var phase = String(run.phase || '').toLowerCase(), winner = run.selectedWinner || (run.serviceVerdict || {}).winner;
		if (winner || run.applyAllowed === true) return Z2M.ui.NoticeBanner({ level: 'success', message: _('Найдена подтверждённая стратегия') });
		if (phase === 'stopped' || phase === 'cancelled' || phase === 'canceled') return Z2M.ui.NoticeBanner({ level: 'warning', message: _('Проверка остановлена. Уже полученные результаты сохранены.') });
		if (phase === 'timed-out' || phase === 'timeout') return Z2M.ui.NoticeBanner({ level: 'warning', message: _('Время проверки истекло. Уже полученные результаты сохранены.') });
		if (phase === 'interrupted' || phase === 'stale') return Z2M.ui.NoticeBanner({ level: 'warning', message: _('Предыдущая проверка не была корректно завершена. Сохранённые результаты доступны ниже.') });
		if (phase === 'infrastructure-error' || phase === 'failed' && run.error && String(run.error.code || '').toLowerCase().indexOf('infra') >= 0) return Z2M.ui.NoticeBanner({ level: 'error', message: _('Инфраструктура недоступна: проверку не удалось завершить. Стратегии не были помечены нерабочими без подтверждения.') });
		if (terminalRun(phase)) return Z2M.ui.NoticeBanner({ level: 'warning', message: _('Подтверждённая рабочая стратегия не найдена. Текущая конфигурация не изменена.') });
		return E('span', {});
	},
	_advancedDiagnostics: function () {
		var s = this._state, a = s.adaptive || {}, caps = s.caps || {}, runtime = a.runtimeSummary || {}, engine = a.engine || a.engineInArgv || {}, raw = a.autohostlistRaw || a.autohostlist || {}, rows = [kv(_('Состояние runtime'), runtime.status || _('Состояние неизвестно')), kv(_('Движок zapret-auto.lua'), engine.auto ? _('Загружен (loaded)') : _('Не загружен')), kv(_('Версия lua_compat'), a.luaCompatVer == null ? _('Недоступна') : a.luaCompatVer + ' (lua_compat_ver)'), kv(_('Матрица возможностей'), caps.matrix ? _('Получена') : _('Не получена')), kv(_('История событий'), s.legacyEvents && s.legacyEvents.available === false ? _('Недоступна') : _('Доступна')), kv(_('История запусков'), s.legacyHistory && s.legacyHistory.available === false ? _('Недоступна') : _('Доступна'))];
		Object.keys(raw).forEach(function (key) { rows.push(kv(key, raw[key])); });
		(caps.matrix || []).forEach(function (item) { if (item.reason) rows.push(kv(item.capability || _('Возможность'), autoText(item.reason, 180))); (item.evidence || []).forEach(function (evidence) { rows.push(kv(_('Доказательство'), autoText(evidence, 180))); }); });
		if (caps.matrix && caps.matrix.length) rows.push(kv(_('Ограничение upstream'), _('Доступны только проверки в памяти процесса; внешние preload API отсутствуют. IN-PROCESS MEMORY ONLY; preload APIs do NOT exist.')));
		if (s.legacyHistory && s.legacyHistory.available === false) rows.push(kv(_('Причина недоступности истории'), autoText(s.legacyHistory.reason, 120)), kv(_('Доказательства'), autoText((s.legacyHistory.evidence || []).join('; '), 180)), kv(_('Версия upstream'), autoText(s.legacyHistory.upstreamVersion, 64)));
		if (s.legacyEvents && s.legacyEvents.available === false) rows.push(kv(_('Причина недоступности событий'), autoText(s.legacyEvents.reason, 120)));
		return details(_('Расширенная диагностика'), E('div', { 'class': 'z2m-auto-diagnostics' }, rows));
	},
	_autoStrategySection: function () {
		var self = this, s = this._state, auto = s.auto, body = E('div', { 'class': 'z2m-auto-page' });
	...
	_autoSection: function () { return this._autoStrategySection(); },
	...
	_autoMutation: function (b, method, label, payload) {
		...
		rpcCall(method, pack(payload)).then(function (response) {
			...
			self._state.autoPending = null; self._state.autoOutcome = response.accepted ? _('Запрос принят; ожидается подтверждение роутера.') : response.status === 'disabled' ? _('Автоподбор отключён.') : response.status === 'already-current' ? _('Последняя рабочая стратегия уже применена.') : response.status ? _('Состояние обновлено; подтвердите результат обновлением.') : _('Операция завершена; статус обновляется.');
			if (response.status === 'disabled') self._state.autoOutcome = _('Автоподбор отключён.');
			if (response.status === 'disable-pending-safe-completion') self._state.autoOutcome = _('Отключение ожидает безопасного завершения.');
			if (response.status === 'cancellation-requested') self._state.autoOutcome = _('Остановка запрошена; ожидается итоговый статус.');
			if (response.status === 'stopped-pending-candidate' || response.status === 'not-running') self._state.autoOutcome = _('Активная проверка не найдена.');
			if (response.status === 'already-current') self._state.autoOutcome = _('Последняя рабочая стратегия уже применена.');
			if (label === _('Запустить проверку') && response.accepted) { self._state.autoOutcome = _('Запрос принят; ожидается подтверждение роутера.'); self._state.autoPoll = true; self._startPolling(); }
			...
			if (self._autoAccessDenied(error)) { self._state.autoReadOnly = true; self._state.autoOutcome = _('Эта сессия доступна только для чтения.'); }
			else if (text.indexOf('ECONFLICT') >= 0) { /* ECONFLICT — re-read the current status */ self._state.autoOutcome = _('Состояние автоподбора изменилось на роутере; обновите статус.'); }
			else { /* could not be confirmed; refresh status */ self._state.autoOutcome = _('Действие автоподбора не подтверждено; обновите статус.'); }
			self._busy(b, label, true); return self._autoRefresh();
		});
	},

	_adaptiveSection: function () { return section(_('Расширенная диагностика'), 'orchestra-adaptive', this._advancedDiagnostics(), _('Только чтение; действия выполняются на странице автоподбора.')); },
