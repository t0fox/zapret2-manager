...
return L.view.extend({
	...
	_runDetail: function (run) {
		if (run.targetType === 'service') return E('div', {}, [this._serviceRunDetail(run), this._serviceProgress(run), this._serviceActions(run), this._state.preview && this._state.preview.runId === run.runId ? this._previewCard() : '']);
	...
	_startPolling: function () { var self = this; if (this._polling || !this._shouldPoll()) return; this._polling = true; this._poll = setInterval(function () { if (!self._shouldPoll()) { self._stopPolling(); return; } var run = self._state.activeRun, activePanel = self._panel === 'orchestra-find' || self._panel === 'orchestra-results'; if (activePanel && run && !terminalRun(run.phase, (self._state.caps || {}).terminalPhases)) { var id = run.runId; rpcCall(runStatusRpc, pack({ runId: id })).then(function (x) { if (x.run) { self._state.activeRun = x.run; if (self._state.selectedRunId === id) self._state.selectedRun = x.run; } self._refresh(); if (!self._shouldPoll()) self._stopPolling(); }).catch(function (e) { self._state.error = structuredError(e); self._refresh(); self._stopPolling(); }); } else if (self._panel === 'orchestra-results' && self._state.operation && !terminalApply(self._state.operation.phase)) rpcCall(applyStatusRpc, pack({ operationId: self._state.operation.operationId })).then(function (x) { if (x.operation) self._state.operation = x.operation; self._refresh(); if (!self._shouldPoll()) self._stopPolling(); }).catch(function (e) { self._state.error = structuredError(e); self._refresh(); self._stopPolling(); }); }, 2000); },
