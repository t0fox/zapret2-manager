...
function normalizeHostlistEntry(value) { var normalized = String(value == null ? '' : value).trim().toLowerCase(), strict = normalized.indexOf('^') === 0; if (strict) normalized = normalized.slice(1); ['https://', 'http://', '//'].some(function (prefix) { if (normalized.indexOf(prefix) === 0) { normalized = normalized.slice(prefix.length); return true; } return false; }); normalized = normalized.split('/')[0].split('?')[0].split('#')[0]; if (normalized.indexOf(':') >= 0 && normalized.charAt(0) !== '[') normalized = normalized.slice(0, normalized.lastIndexOf(':')); if (normalized.indexOf('www.') === 0) normalized = normalized.slice(4); var result = domain(normalized.replace(/\.+$/, '')); return result ? (strict ? '^' : '') + result : null; }
