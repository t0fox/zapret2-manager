...
function load(ctx) {
  var eventsCall = state.eventsUnsupported ? Promise.resolve({ unsupported: true }) : edit(ctx.api.monitor.eventsTail, {});
  return Promise.allSettled([ctx.api.monitor.status(), eventsCall]).then(function (results) {
    var data = {};
    if (results[0].status === 'fulfilled') {
      data.status = results[0].value || {};
      state.lastGood = data.status;
    } else {
      data.statusError = ctx.api.normalizeError(results[0].reason);
    }
    if (results[1].status === 'fulfilled') {
      if (results[1].value && results[1].value.unsupported === true) data.eventsUnsupported = true;
      else data.events = results[1].value || {};
    } else {
      data.eventsError = ctx.api.normalizeError(results[1].reason);
      state.eventsUnsupported = unsupported(data.eventsError);
      data.eventsUnsupported = state.eventsUnsupported;
    }
    return data;
  });
}
...
function renderMonitor(ctx) {
  ...
  nodes.push(E('div', { 'class': 'z2m-phead' }, [
    E('div', {}, [E('h1', {}, _('Мониторинг')), E('p', {}, _('Live runtime state; обновление каждые 5 секунд только на активной вкладке'))])
  ]));
  ...
  if (state.pane === 'connections') {
    ...
    nodes.push(shell.panel(
      _('nfqws2 instances'),
      rows.length ? table([_('PID'),_('qnum'),_('RSS KiB'),_('Command')], rows) : shell.empty(_('No nfqws2 instances running.')),
      queue.registered === false ? _('NFQUEUE not registered') : _('read-only runtime')
    ));
  } else if (state.pane === 'diagnostics') {
    var checkRows = checks.slice(0, 20).map(function (check) {
      return E('tr', {}, [E('td', {}, show(check.id || check.name)), E('td', {}, E('code', {}, JSON.stringify(check)))]);
    });
    var jobRows = jobs.slice(0, 20).map(function (job) {
      return E('tr', {}, [E('td', {}, show(job.id)), E('td', {}, show(job.status)), E('td', {}, show(job.updatedAt))]);
    });
    ...
    warnings.slice(0, 10).forEach(function (warning) {
      nodes.push(E('div', { 'class': 'warnbar' }, show(warning.code) + ': ' + show(warning.message)));
    });
  } else {
    var events = data.events || {}, lines = asArray(events.events || events.lines || events.items);
    if (state.eventsUnsupported || data.eventsUnsupported) {
      nodes.push(shell.panel(_('Журнал службы'), E('div', { 'class': 'z2m-empty' }, _('События недоступны: установленный backend не предоставляет events_tail.'))));
    } else if (data.eventsError) {
      nodes.push(shell.panel(_('Журнал службы'), E('div', { 'class': 'warnbar' }, data.eventsError.message)));
    } else {
      nodes.push(shell.panel(_('Журнал службы'), E('pre', { 'class': 'z2m-console' }, lines.map(function (entry) {
...
function render(ctx) {
  return E('section', { 'class': 'z2m-view on', id: 'z2m-view-monitor' }, renderMonitor(ctx));
}
function mount(ctx) {
  ...
  state.timer = setInterval(function () {
    ...
    ctx.api.monitor.status().then(function (status) {
      ...
      ctx.data.statusError = null;
      state.lastGood = ctx.data.status;
      ctx.root.replaceChildren.apply(ctx.root, renderMonitor(ctx));
    }).catch(function (error) {
      ctx.data.statusError = ctx.api.normalizeError(error);
      ctx.root.replaceChildren.apply(ctx.root, renderMonitor(ctx));
    }).then(function () {
