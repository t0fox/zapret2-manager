...
// Service DNS Apply waits for the backend worker to finish its bounded
// transaction (up to 30 seconds). Keep the RPC transport open long enough to
// receive the real result; the view still verifies service_dns_status.applied.
function z2kServiceDnsApply(value) {
  return z2kLongMutation('service_dns_apply', { edit: value }).then(function (answer) {
    if (!answer || answer.ok === false || answer.error) {
      var errorValue = answer && answer.error != null ? answer.error : answer;
      var details = errorValue && typeof errorValue === 'object' ? errorValue : {};
      var error = z2kRpcError(details.code || answer && answer.code || 'RPCError',
        details.message || (typeof errorValue === 'string' ? errorValue : answer && answer.message) || 'Service DNS Apply failed');
      if (answer && typeof answer === 'object') Object.keys(answer).forEach(function (key) {
        if (key !== 'error' && error[key] === undefined && answer[key] !== undefined) error[key] = answer[key];
      });
      if (answer && answer.operationId !== undefined) error.operationId = answer.operationId;
      if (answer && answer.error !== undefined) error.error = answer.error;
      throw error;
    }
    return answer;
  });
...
serviceDnsStatus:rpc.declare({object:'zapret2-manager',method:'service_dns_status',reject:true}),serviceDnsSet:rpc.declare({object:'zapret2-manager',method:'service_dns_set',params:['edit'],reject:true}),serviceDnsApply:z2kServiceDnsApply,serviceDnsApplyStatus:rpc.declare({object:'zapret2-manager',method:'service_dns_apply_status',params:['edit'],reject:true}),serviceDnsTiktokSetAsync:rpc.declare({object:'zapret2-manager',method:'service_dns_tiktok_set_async',params:['edit'],reject:true}),serviceDnsTiktokStatus:rpc.declare({object:'zapret2-manager',method:'service_dns_tiktok_status',reject:true}),serviceDnsTiktokCheck:rpc.declare({object:'zapret2-manager',method:'service_dns_tiktok_check',params:['edit'],reject:true}),serviceDnsRollback:rpc.declare({object:'zapret2-manager',method:'service_dns_rollback',reject:true}),
