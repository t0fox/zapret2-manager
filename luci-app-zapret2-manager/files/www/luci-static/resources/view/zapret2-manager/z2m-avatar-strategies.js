...
var ListUI = {
  create: function (options) {
    ...
    return {
      ...
      refresh: refresh,
      destroy: function () { listeners.forEach(function (entry) { entry[0].removeEventListener(entry[1], entry[2]); }); if (observer) observer.disconnect(); root.classList.remove('list-ui'); }
    };
  }
};
...
function showPreview(id) { var strategy = strategyById(id); if (!strategy) return; state.preview = { strategy: strategy, validation: null, output: 'Загрузка…', pending: true }; renderPreviewModal(); state.root.querySelector('#preview-modal').style.display = 'flex'; call(state.ctx.api.strategies.preview, previewRequest(strategy, state.data, false)).then(function (answer) { state.preview.pending = false; state.preview.output = text(answer && (answer.effectiveCommand || answer.command || answer.output)) || 'Backend не вернул команду'; renderPreviewModal(); }).catch(function (error) { state.preview.pending = false; state.preview.output = errorText(state.ctx, error); renderPreviewModal(); }); }
...
function onClick(event) {
  ...
  else if (action === 'editorPreview') { collectEditor(); var output = state.root.querySelector('#editor-preview-output'); output.style.display = 'block'; output.textContent = 'Загрузка…'; call(state.ctx.api.strategies.preview, previewRequest(state.editor.strategy, state.data, false)).then(function (answer) { output.textContent = text(answer && (answer.effectiveCommand || answer.command || answer.output)) || 'Backend не вернул команду'; }).catch(function (error) { output.textContent = errorText(state.ctx, error); }); }
