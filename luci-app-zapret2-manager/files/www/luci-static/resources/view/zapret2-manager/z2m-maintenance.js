...
function load(ctx) {
  return Promise.allSettled([
    ...
    edit(ctx.api.maintenance.eventsTail, {})
...
function renderMaintenance(ctx) {
  ...
  var versions = data.versions && data.versions.value || {};
  var system = data.status && data.status.value || {};
  var backups = data.backups && data.backups.value || {};
  var scopes = backups.scopes || {};
  var nodes = [];
  ...
  function confirmAction(title, message, confirmLabel, handler) {
    var cancelButton = shell.button(_('Отмена'), '', shell.closeModal);
    var confirmButton = shell.button(confirmLabel, 'danger', function () {
      shell.closeModal();
      handler();
    });
    shell.openModal(title, E('p', {}, message), [cancelButton, confirmButton]);
  }

  nodes.push(E('div', { 'class': 'z2m-phead' }, [
    E('div', {}, [E('h1', {}, _('Обслуживание')), E('p', {}, _('Версии, scoped backups, события и diagnostics export'))])
  ]));
  ...
  ['all'].concat(SCOPES).forEach(function (scope) {
    scopeSelect.appendChild(E('option', { value: scope }, scope));
  });
  var createButton = shell.button(_('Create backup'), 'primary', function () {
    ...
    edit(ctx.api.maintenance.backupCreate, { scope: scopeSelect.value || 'all' }).then(function (answer) {
      ...
      shell.showToast(_('Backup created.'), 'ok');
      return reload();
    ...
    records.forEach(function (record) {
      ...
      var previewButton = shell.button(_('Preview'), 'sm', function () {
        ...
        edit(ctx.api.maintenance.backupPreview, { scope: scope, takenAt: takenAt }).then(function (result) {
          ...
          state.preview = { scope: scope, takenAt: takenAt, error: ctx.api.normalizeError(error) };
          rerender();
        });
      });
        ...
        confirmAction(
          ...
          function () {
            edit(ctx.api.maintenance.backupDelete, { scope: scope, takenAt: takenAt }).then(function (answer) {
              ...
              shell.showToast(_('Backup deleted.'), 'ok');
              return reload();
            }).catch(showError);
          }
        );
      ...
      history.appendChild(E('div', { 'class': 'z2m-backup-row' }, [
        E('div', {}, [
          E('strong', {}, formatTime(takenAt)),
          E('div', { 'class': 'z2m-dim' }, show(record.manifestSha256))
        ]),
  ...
  nodes.push(shell.panel(
    _('Backups'),
    E('div', {}, [E('div', { 'class': 'z2m-btnrow' }, [scopeSelect, createButton]), history]),
    _('Four independent scopes; backend enforces retention and SHA-256 manifests.')
  ));
  ...
  if (state.preview) {
    ...
    var body;
    if (pv.error) {
      body = E('div', { 'class': 'warnbar' }, _('Preview failed: ') + pv.error.message);
    } else if (result.ok !== true) {
      body = E('div', { 'class': 'warnbar' }, _('Preview refused: ') + show(result.error && result.error.message || result.error));
    } else {
      var restoreButton = shell.button(_('Restore this archive'), 'danger', function () {
        confirmAction(
          _('Восстановить backup?'),
          _('Текущее состояние будет сохранено backend перед восстановлением.'),
          _('Восстановить'),
          function () {
            edit(ctx.api.maintenance.backupRestore, { scope: pv.scope, takenAt: pv.takenAt }).then(function (answer) {
              ...
              state.preview = null;
              shell.showToast(_('Backup restored.'), 'ok');
              return reload();
            }).catch(showError);
          }
        );
    ...
    nodes.push(E('section', { 'class': 'z2m-panel', id: 'z2m-backup-preview' }, [
      E('div', { 'class': 'hd' }, E('h2', {}, _('Restore preview'))),
      E('div', { 'class': 'bd' }, body)
    ]));
  }

  var events = data.events && data.events.value || {};
  var eventLines = asArray(events.events || events.lines || events.items);
  nodes.push(shell.panel(
    _('Events'),
    data.events && data.events.error
      ? E('div', { 'class': 'warnbar' }, data.events.error.message)
      : E('pre', { 'class': 'z2m-console' }, eventLines.map(function (item) {
  ...
  var exportButton = shell.button(_('Diagnostics export'), '', function () {
    ...
    ctx.api.maintenance.diagnosticsExport().then(function (answer) {
      diagnosticResult.textContent = JSON.stringify(answer, null, 2);
...
function render(ctx) {
  return E('section', { 'class': 'z2m-view on', id: 'z2m-view-maintenance' }, renderMaintenance(ctx));
}
function mount() {}
function unmount() {}
return {
