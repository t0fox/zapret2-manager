...
export const UI_FILES = [
  'orchestra-strategy.js', 'orchestra.js', 'strategies.js', 'services.js', 'lists.js',
  'dns.js', 'monitor.js', 'proxy.js', 'maintenance.js'
];

// Engine declarations are created lazily by the central facade so loading the
// single-view shell does not fail on older backends. They are nevertheless a
// frozen part of that facade and must participate in contract drift checks.
export const LAZY_FACADE_METHODS = [
...
export const UI_CONTRACT = {
  ...
  'services.js': ['domain_hub_apply','domain_hub_get','domain_hub_preview'],
  ...
  'monitor.js': ['monitor_snapshot','status'],
  ...
  'maintenance.js': ['backup_create','backup_delete','backup_list','backup_restore','backup_restore_preview','diagnostics_export','engine_check_updates','engine_install','engine_operation_cancel','engine_operation_status','engine_providers','engine_remove','engine_status','events_tail','maintenance_status','versions']
};
...
export function collectFacadeMethods(root = process.cwd()) {
  const source = readFileSync(resolve(viewBase(root), 'z2m-api.js'), 'utf8');
  return [...new Set(extractRpcMethods(source).concat(LAZY_FACADE_METHODS))].sort();
}
...
export function collectUiContract(root = process.cwd()) {
  ...
  if (existsSync(facade)) {
    const methods = new Set(collectFacadeMethods(root));
