...
const DEBUG_FRONTEND=new Set(['tests/engine-providers.test.mjs','tests/engine-state-machine.test.mjs','tests/flowseal-combo-apply.test.mjs']);
...
function isFrontendTest(file){return regularFrontend(file)||DEBUG_FRONTEND.has(file);}
...
const selected=discovered.filter(belongsToGroup);if(selected.length===0){console.error(`Refusing to pass: no ${group} tests were discovered.`);process.exit(2);}
