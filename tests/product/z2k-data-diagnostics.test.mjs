...
test('truthy production renames require source and destination postconditions', { skip: !ucode || !fs.existsSync(dataPath) }, () => {
  const input = coreInput();
  const identity = invoke(dataPath, `mod.z2k_data_identity(${JSON.stringify(input)})`);
  const results = ['stage-rename', 'pointer-rename'].map(mode => ({ mode, result: invoke(dataPath, productionFailureExpression(identity, mode)) }));
  for (const { mode, result } of results) {
    assert.equal(result.result.ok, false, JSON.stringify({ mode, result }));
    assert.equal(result.result.error.code, 'EROLLBACK_FAILED', JSON.stringify({ mode, result }));
    assert.equal(result.result.state, 'uncertain', JSON.stringify({ mode, result }));
    assert.match(JSON.stringify(result.result), /ERENAME_VERIFY/, JSON.stringify({ mode, result }));
    assert.equal(result.oldPointer, '{"schema":1,"revision":"old"}', JSON.stringify({ mode, result }));
    assert.deepEqual(result.revisions, [], JSON.stringify({ mode, result }));
  }
});
