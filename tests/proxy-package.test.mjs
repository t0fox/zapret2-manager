...
//
// The SHA-256 pin is asserted to be IDENTICAL in every source of truth:
// tg-ws-proxy-rs/Makefile, tests/lib/proxy-logic.mjs (PROVIDER_PROFILE),
// and the shipped proxy.uc — a drift between them would be a silent
// trust-anchor fork.
//
...
const BACKEND_UC = join(ROOT, 'zapret2-manager', 'files', 'usr', 'libexec', 'zapret2-manager', 'proxy.uc');

const PIN = {
	version: '1.6.5',
	release: 'v1.6.5',
	sha256: '54803f09f9b4a83b27e7d6fa2dd7bbeb51df04d6365f29b5746086d2830dc45a',
	asset: 'tg-ws-proxy-aarch64-unknown-linux-musl.tar.gz',
	commit: 'a14a97aee20a1da428eb7dbd5fbe23195eba0b9d'
};
...
test('the SHA-256 pin is identical across Makefile, node reference and shipped ucode', () => {
	const mk = readFileSync(MAKEFILE, 'utf8');
	const logic = readFileSync(join(HERE, 'lib', 'proxy-logic.mjs'), 'utf8');
	const uc = readFileSync(BACKEND_UC, 'utf8');
	assert.ok(mk.includes(PIN.sha256), 'Makefile must carry the pin');
	assert.ok(logic.includes(PIN.sha256), 'proxy-logic.mjs must carry the pin');
	assert.ok(uc.includes(PIN.sha256), 'proxy.uc must carry the pin');
});

// ---- negative controls ----------------------------------------------------------

test('NEGATIVE CONTROL: a wrong PKG_HASH reddens the Makefile gate', () => {
	const broken = readFileSync(MAKEFILE, 'utf8').replace(PIN.sha256, PIN.sha256.replace('54803f', 'ffffff'));
