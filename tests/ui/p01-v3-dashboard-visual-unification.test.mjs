...
test('P01-V3 Dashboard uses the shared SVG registry and action geometry contract', () => {
  const dashboard = read('z2m-avatar-dashboard.js');
  const avatarUi = read('z2m-avatar-ui.js');
  ...
  for (const name of ['play', 'stop-square', 'rotate-cw']) {
    assert.match(icons, new RegExp(`(?:^|\\n)\\s*(?:['"]${name}['"]|${name})\\s*:`), `missing shared icon glyph: ${name}`);
  }
  for (const marker of ['control-button-icon-slot', 'control-button-label']) {
    assert.match(combined, new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')), `missing action marker: ${marker}`);
  }
  assert.match(css, /\.actions-row \.btn-lg\{[^}]*height:32px/);
  assert.match(css, /\.control-button-icon-slot\{[^}]*width:14px/);
  assert.match(css, /\.control-button-label\{/);
});
