...
test('Control model has conservative STOPPED, RUNNING, and UNKNOWN states', () => {
  ...
  assert.equal(JSON.stringify(model.buttons('stopped', false)), JSON.stringify({ start: false, stop: true, restart: true }));
  assert.equal(JSON.stringify(model.buttons('running', false)), JSON.stringify({ start: true, stop: false, restart: false }));
  assert.equal(JSON.stringify(model.buttons('unknown', false)), JSON.stringify({ start: true, stop: true, restart: true }));
  assert.equal(JSON.stringify(model.buttons('running', true)), JSON.stringify({ start: true, stop: true, restart: true }));
});
