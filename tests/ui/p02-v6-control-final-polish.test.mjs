...
test('P02-V6 manual transitions suppress only bounded convergence, not real loss', () => {
  ...
  assert.doesNotMatch(constants, /MANUAL_TRANSITION_TTL_SEC|manual_transition/);
  assert.match(service, /verify_engine_runtime\('start'/);
  assert.match(service, /verify_engine_runtime\('restart'/);
  assert.match(service, /reason: 'manual_ui'/);
  assert.match(watchdog, /reason: 'process_crash'/);
  assert.match(watchdog, /alert_if\('process_gone'/);
  assert.match(watchdog, /reason: 'queue_length'/);
