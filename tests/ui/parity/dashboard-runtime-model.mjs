/**
 * Small, dependency-free model of the Dashboard runtime contract.
 *
 * This is deliberately not a second production implementation. It gives the
 * regression suite a deterministic way to prove that one failed or stalled
 * optional read is represented as a bounded envelope and cannot reject the
 * independent status/events result.
 */
...
export function settleRead(read, { timeoutMs = 100, label = 'read' } = {}) {
  ...
  const deadline = new Promise((resolve) => {
    timer = setTimeout(() => resolve({
      state: 'TIMEOUT',
      label,
      error: `${label} exceeded ${timeoutMs}ms budget`,
      elapsedMs: Date.now() - started,
    }), timeoutMs);
  });
...
export async function collectStatusProjection(parts, options) {
  const entries = await Promise.all(Object.entries(parts).map(async ([label, read]) => [
    label,
    await settleRead(read, { ...options, label }),
  ]));
  const fields = {};
  const degraded = [];
