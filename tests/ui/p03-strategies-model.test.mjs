...
test('Strategies model maps safe Russian states and hides raw backend fields', () => {
  ...
  assert.equal(model.visibleReason({ reasonCode: 'candidate-invalid', status: 'failed' }), '');
});

test('Strategies model blocks duplicate mutations and unsupported donor-only operations', () => {
  const model = loadModel();
  assert.equal(model.canMutate(false), true);
  assert.equal(model.canMutate(true), false);
  assert.equal(model.classifyUnsupported('healthcheck'), 'BACKEND_NOT_READY');
  assert.equal(model.classifyUnsupported('autocircular'), 'BACKEND_NOT_READY');
});
