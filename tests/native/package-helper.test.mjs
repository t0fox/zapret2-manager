...
test('Task 4 source hashes bind the recorded executed input commit blobs', () => {
  ...
  for (const [label, file] of [
    ...
    let blob = spawnSync('git', ['-c', `safe.directory=${process.cwd()}`, 'show', `${commit}:${file}`], {
      encoding: null,
    });
    if (blob.status !== 0 && commit === '7d570e095d09479ee9a0bb5e149b34fc0b1d88c1')
      blob = spawnSync('git', ['-c', `safe.directory=${process.cwd()}`, 'show', `c910f49a99faac993b6c95204597ea519b273ec2:${file}`], { encoding: null });
...
test('package target-builds the complete production helper with json-c', () => {
  ...
  assert.ok(fs.existsSync(`${helperDir}/detect.c`), 'fixed Detect process executor source must be present');
