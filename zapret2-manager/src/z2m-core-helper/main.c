...
int main(void)
{
	struct z2m_request request={0};json_object *root_value;const char *root_name,*code;const struct z2m_root *root;int fd,result=z2m_read_request(&request);
	if(result!=-1){z2m_request_free(&request);return result;}
	if(strcmp(request.operation,"stat_regular")!=0&&strcmp(request.operation,"read_regular")!=0&&strcmp(request.operation,"mkdir_private")!=0){
	...
	if(strcmp(request.operation,"mkdir_private")==0&&!z2m_reserved_schema_valid(&request)){result=z2m_fail(request.request_id,"ESCHEMA","schema");z2m_request_free(&request);return result;}
	...
	if((strcmp(request.operation,"stat_regular")==0&&!root->stat_allowed)||(strcmp(request.operation,"read_regular")==0&&!root->read_allowed)||(strcmp(request.operation,"mkdir_private")==0&&!root->mkdir_allowed)){result=z2m_fail(request.request_id,"EDENIED","policy");z2m_request_free(&request);return result;}
	fd=z2m_root_open(root);if(fd<0){result=z2m_fail(request.request_id,"EROOT","root_open");z2m_request_free(&request);return result;}
	if(strcmp(request.operation,"mkdir_private")==0){if(z2m_root_lock(fd,&code)<0)result=z2m_fail(request.request_id,code,"lock_acquire");else result=z2m_mkdir_private(&request,root,fd);}
