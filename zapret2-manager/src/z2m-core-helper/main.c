...
int main(void)
{
	...
	if(strcmp(request.operation,"mkdir_private")==0){if(z2m_root_mount_id(fd,&root_mount,&code)<0)result=z2m_fail(request.request_id,code,"path_resolve");else if(z2m_root_lock(fd,&code)<0)result=z2m_fail(request.request_id,code,"lock_acquire");else result=z2m_mkdir_private(&request,root,fd,root_mount);}
