...
int main(void)
{
	...
	if(strcmp(request.operation,"stat_regular")!=0&&strcmp(request.operation,"read_regular")!=0&&strcmp(request.operation,"atomic_write")!=0&&strcmp(request.operation,"mkdir_private")!=0&&strcmp(request.operation,"sha256_regular")!=0){
	...
	if((strcmp(request.operation,"atomic_write")==0||strcmp(request.operation,"mkdir_private")==0||strcmp(request.operation,"sha256_regular")==0)&&!z2m_reserved_schema_valid(&request)){result=z2m_fail(request.request_id,"ESCHEMA","schema");z2m_request_free(&request);return result;}
