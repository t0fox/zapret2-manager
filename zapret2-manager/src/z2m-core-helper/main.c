...
int main(void)
{
	...
	else if(strcmp(request.operation,"stat_regular")==0)result=z2m_stat_regular(&request,root,fd);else if(strcmp(request.operation,"read_regular")==0)result=z2m_read_regular(&request,root,fd);else result=z2m_sha256_regular(&request,root,fd);
