...
function hostname(value) { let s = lower_ascii(value), strict = substr(s, 0, 1) == '^'; if (strict) s = substr(s, 1); for (let i = 0; i < 3; i++) { let prefix = ['https://', 'http://', '//'][i]; if (substr(s, 0, length(prefix)) == prefix) s = substr(s, length(prefix)); } s = split(s, '/')[0]; s = split(s, '?')[0]; s = split(s, '#')[0]; if (index(s, ':') >= 0 && substr(s, 0, 1) != '[') s = substr(s, 0, rindex(s, ':')); if (substr(s, 0, 4) == 'www.') s = substr(s, 4); while (substr(s, 0, 1) == '.') s = substr(s, 1); while (substr(s, length(s) - 1, 1) == '.') s = substr(s, 0, length(s) - 1); if (!length(s) || length(s) > 253 || !match(s, /^[a-z0-9]([a-z0-9-]*[a-z0-9])?(\.[a-z0-9]([a-z0-9-]*[a-z0-9])?)*$/)) return null; return (strict ? '^' : '') + s; }
...
// nfqws2 drops privileges before it opens hostlists, ipsets, and fake blobs.
// Registry metadata remains private, but materialized runtime assets must be
// traversable/readable by that daemon UID after every atomic replacement.
function runtime_asset_permissions(kind, path) {
	if (!valid_type(kind) || !string(path) || substr(path, 0, length(USER_ROOT + '/' + kind + '/')) != USER_ROOT + '/' + kind + '/') return false;
	let dirs = command('chmod 0755 ' + shell_quote(USER_ROOT) + ' ' + shell_quote(USER_ROOT + '/' + kind));
	if (dirs.rc != 0) return false;
	let file = command('chmod 0644 ' + shell_quote(path));
	return file.rc == 0 && regular(path);
}
function runtime_asset_permissions_for_path(path) {
	let relative = substr(path, length(USER_ROOT) + 1), slash = index(relative, '/');
	return slash > 0 ? runtime_asset_permissions(substr(relative, 0, slash), path) : false;
}
...
function package_manifest_asset(id) {
	...
	for (let i = 0; i < length(manifest.bundles); i++) for (let j = 0; j < length(manifest.bundles[i].assets || []); j++) {
		...
		return { schema: 1, type: item.type, id: item.id, name: item.name, ownership: 'package', mutable: false,
			...
			byteSize: content_size(item.packagePath), revision: 0, path: item.packagePath, references: [], validation: { status: 'passed', errors: [] } };
	}
	return null;
}
...
export const asset_registry_list = function(kind, includeReceipts) { let state = state_load(); if (state == null) return fail('ESTATE', 'asset registry metadata is invalid'); let assets = []; for (let i = 0; i < length(state.assets); i++) if (kind == null || state.assets[i].type == kind) push(assets, project_asset(state.assets[i])); return { ok: true, schema: 1, revision: state.revision, assets: assets, activationReceipts: includeReceipts === false ? [] : copy_array(state.activationReceipts) }; };
...
export const asset_registry_apply_bundle = function(request) {
	...
	if (result == null) for (let i = 0; i < length(prepared); i++) {
		...
		if (!atomic_write(path, entry.content) || !runtime_asset_permissions(item.type, path) || !postflight(path, item)) { let actualSha = sha256_file(path), actualSize = content_size(path); result = fail('EVERIFY', 'resource activation postflight failed', { id: item.id, expectedSha256: item.sha256, actualSha256: actualSha, expectedSize: item.byteSize, actualSize: actualSize }); break; }
		if (old == null) {
			push(state.assets, { schema: 1, type: item.type, id: item.id, name: item.name || substr(item.id, length(item.type) + 1), ownership: 'manager', mutable: true, provenance: copy(entry.provenance), contentSha256: item.sha256, byteSize: item.byteSize, revision: 1, lastChecked: time(), lastUpdated: time(), path: path, references: [], validation: { status: 'passed', errors: [] } });
...
export const asset_registry_rollback_bundle = function(request) {
	...
	for (let i = 0; i < length(snapshot.records); i++) {
		...
		let restoredOk = record.hadPrevious ? atomic_write(record.path, previous) && runtime_asset_permissions_for_path(record.path) : true;
