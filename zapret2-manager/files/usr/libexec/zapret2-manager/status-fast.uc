...
function runtime_summary(state, rows, queue) {
	let present = length(rows) > 0, registered = queue.registered === true;
	...
	return { schemaVersion: 1, source: 'status-fast.v1', status: summaryState,
		...
		nfqueue: { number: NFQUEUE, registered: registered, ownerMatches: registered ? !queue.ownerConflict : null, rulesPresent: null },
		watchdog: { running: null, lastSeenProcess: null } };
}

function collect() {
	let contract, rows, queue, selected;
