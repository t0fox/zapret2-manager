...
const DNS_MODES = { auto: true, system: true, doh: true };
...
export const blockcheckw_request_validate = function(input) {
	...
	if (!SOURCES[strategy_source]) return fail('EINPUT', 'strategy_source is not supported', 'strategy_source');
	if (input.engine != 'scan' && strategy_source != 'builtin') return fail('EINPUT', 'strategy_source is only supported in scan mode', 'strategy_source');

	let dns = input.dns == null ? 'auto' : input.dns;
	if (!DNS_MODES[dns]) return fail('EINPUT', 'dns must be auto, system, or doh', 'dns');
	...
	for (let key in input) if (key != 'engine' && key != 'domains' && key != 'domain_list' && key != 'strategy_source' && key != 'dns' && key != 'workers' && key != 'protocols' && key != 'timeout' && key != 'passes' && key != 'sample' && key != 'source_job') return fail('EINPUT', 'unknown BlockCheckW field', key);
	...
	return { ok: true, value: { engine: input.engine, strategy_source: strategy_source, dns: dns, domains: ds.value, domain_list: list.value, workers: workers, protocols: protocols, timeout: timeout, passes: passes, sample: sample, source_job: input.source_job || null, auto: true, no_conflict_cleanup: true }, serverOwned: true, argv: [] };
};
