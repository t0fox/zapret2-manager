...
function admission_reason(allowed, code, details) { return { allowed: allowed === true, reasonCode: allowed === true ? null : code, details: details || {} }; }
function admission_reasons(state, verified, recovery, runtime) {
	let busy = state.activeRunId != null || state.pendingApplyRunId != null;
	let runAllowed = state.enabled && !busy && !recovery;
	let runCode = !state.enabled ? 'auto-disabled' : !length(state.serviceIds || []) ? 'no-services-selected' : recovery ? 'recovery-required' : busy ? 'operation-active' : state.cooldownUntil != null ? 'cooldown-active' : runtime.status == 'unknown' ? 'runtime-not-confirmed' : null;
	return {
		enable: admission_reason(!state.enabled, state.enabled ? 'already-enabled' : 'state-corrupt', {}),
		disable: admission_reason(state.enabled, state.enabled ? null : 'already-disabled', {}),
		runNow: admission_reason(runAllowed, runCode || 'operation-active', { serviceCount:length(state.serviceIds || []) }),
		stop: admission_reason(state.activeRunId != null && !recovery, recovery ? 'recovery-required' : 'no-active-operation', {}),
		restoreLastGood: admission_reason(verified && !state.activeRunId && !recovery, !verified ? 'no-last-good' : recovery ? 'recovery-required' : 'operation-active', {})
	};
}
function auto_rpc_status() {
	...
	if (auto_state_corrupt()) return { ok: true, schemaVersion: 1, revision: 0, enabled: false, phase: 'failed', serviceIds: [], consecutiveFailures: 0, activeRun: { runId: null, generation: null, startedAt: null, progress: null, cancellable: false }, currentApplied: { revision: null, hash: null }, lastGood: { available: false, candidateId: null, profileRevision: null, profileHash: null, appliedAt: null }, health: { status: 'unknown', lastCheckAt: null, lastSuccessAt: null, lastFailureAt: null }, infrastructure: { status: 'failed', reason: 'state-corrupt' }, cooldownUntil: null, lastError: 'state-corrupt', capabilities: { runNow: false, stop: false, restoreLastGood: false }, admissionReasons: { enable: admission_reason(false, 'state-corrupt', {}), disable: admission_reason(false, 'state-corrupt', {}), runNow: admission_reason(false, 'state-corrupt', {}), stop: admission_reason(false, 'state-corrupt', {}), restoreLastGood: admission_reason(false, 'state-corrupt', {}) }, runtimeSummary: runtime_summary_cached(), verifyRouter: [] };
