...
function dataset() {
	try {
		let d = json(readfile(DATASET_PATH)), catalog = dns_provider_catalog_get();
		if (d && type(d.profiles) == 'array' && catalog.ok === true && type(catalog.providers) == 'array') {
			d.providers = catalog.providers;
			d.providerRevision = catalog.revision;
			return { ok: true, data: d };
		}
		if (catalog.ok !== true) return { ok: false, error: catalog.error };
	} catch (e) {}
	return { ok: false };
}
...
function maps(d) {
	let p = {}, v = {}, serviceProfiles = {}, serviceDomains = {};
	for (let i = 0; i < length(d.profiles); i++) {
		let profile = d.profiles[i];
		p[profile.id] = profile;
		if (serviceProfiles[profile.serviceId] == null) serviceProfiles[profile.serviceId] = {};
		serviceProfiles[profile.serviceId][profile.providerId] = profile;
		if (serviceDomains[profile.serviceId] == null) serviceDomains[profile.serviceId] = copy(profile.requiredDomains || []);
	}
	for (let j = 0; j < length(d.providers); j++) v[d.providers[j].id] = d.providers[j];
	return { profiles: p, providers: v, serviceProfiles: serviceProfiles, serviceDomains: serviceDomains };
}
function resolve_selection(m, serviceId, selected) {
	if (selected == null || selected == '' || selected == 'off') return null;
	let legacy = m.profiles[selected];
	if (legacy) return { providerId: legacy.providerId, profile: legacy };
	let provider = m.providers[selected], profile = m.serviceProfiles[serviceId] && m.serviceProfiles[serviceId][selected];
	if (!provider) return null;
	return { providerId: selected, profile: profile || { serviceId: serviceId, providerId: selected, requiredDomains: m.serviceDomains[serviceId] || [] } };
}
function canonical_selections(selections, d) {
	...
	return { selections: result, invalid: invalid };
}
function normalize_state(state, d) {
	let mapped = canonical_selections(state.selections || {}, d);
	if (length(mapped.invalid)) return err('EINPUT', 'service DNS selection references an unavailable provider or service profile', { invalid: mapped.invalid });
	state.selections = mapped.selections;
	if (state.applied && type(state.applied.selections) == 'object') {
		let applied = canonical_selections(state.applied.selections, d);
		if (length(applied.invalid)) return err('EINPUT', 'applied service DNS state references an unavailable provider or service profile', { invalid: applied.invalid });
		state.applied.selections = applied.selections;
	}
	return { ok: true, state: state };
}
function desired_routes(selections, d) {
	let mapped = canonical_selections(selections, d), m = maps(d), byDomain = {}, conflicts = [], entries = [], seen = {};
	selections = mapped.selections;
	for (let service in selections || {}) {
		let resolved = resolve_selection(m, service, selections[service]);
		if (!resolved) continue;
		let provider = m.providers[resolved.providerId] || {}, profile = resolved.profile;
		for (let i = 0; i < length(profile.requiredDomains || []); i++) {
			let domain = lc(profile.requiredDomains[i]);
			if (byDomain[domain] && byDomain[domain] != provider.id) {
				push(conflicts, { domain: domain, providers: [byDomain[domain], provider.id], services: [service] });
				continue;
			}
			byDomain[domain] = provider.id;
			for (let j = 0; j < length(provider.ipv4 || []); j++) {
				let entry = route_entry(domain, provider.ipv4[j]);
				if (entry && !seen[entry]) { seen[entry] = true; push(entries, entry); }
			}
		}
	}
	sort(entries);
	return { entries: entries, conflicts: conflicts, providers: byDomain, invalid: mapped.invalid };
}
