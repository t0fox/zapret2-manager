...
function process_identity(pid) {
	...
	return { pid: pid, startTime: start, exe: readlink('/proc/' + pid + '/exe') || '', cmdline: proc_cmdline(pid) };
}
...
function public_job(job) { if (!job) return null; return { id: job.id, product: 'blockcheck2', status: job.status, phase: job.phase, mode: job.request.mode, strategy_source: job.request.strategy_source, domains: job.request.domains, options: job.request.options, script: job.script, progress: job.progress, total: job.total, startedAt: job.startedAt, finishedAt: job.finishedAt, exitCode: job.exitCode, error: job.error, recovery: job.recovery, outputCursor: job.outputCursor || 0, parsed: job.parsed || null }; }
...
export const blockcheck2_start = function(input) {
	...
	let id = 'bc2-' + time() + '-' + seq(), request = { mode: value.mode, strategy_source: built.strategy_source, domains: built.domains, options: object(value.options) ? value.options : {} };
	...
	let envVars = {};
	for (let key in built.env) envVars[key] = built.env[key];

	if (built.strategy_source == 'catalog_quick' || built.strategy_source == 'catalog_standard') {
		let set = built.strategy_source == 'catalog_quick' ? 'quick' : 'standard';
		let tcpList = catalog.strategy_catalog_list('tcp', set);
		let udpList = catalog.strategy_catalog_list('udp', set);

		let tcpLines = [];
		for (let i = 0; i < length(tcpList); i++) {
			let entry = tcpList[i];
			let a = entry.args || entry.raw || (entry.profiles ? entry.profiles[0]?.args : '') || '';
			if (a) push(tcpLines, a);
		}
		let udpLines = [];
		for (let i = 0; i < length(udpList); i++) {
			let entry = udpList[i];
			let a = entry.args || entry.raw || (entry.profiles ? entry.profiles[0]?.args : '') || '';
			if (a) push(udpLines, a);
		}

		let serTcp = model.blockcheck2_custom_list_serialize(tcpLines);
		let serUdp = model.blockcheck2_custom_list_serialize(udpLines);

		let tls12File = path(id, '-tls12.txt');
		let tls13File = path(id, '-tls13.txt');
		let httpFile = path(id, '-http.txt');
		let quicFile = path(id, '-quic.txt');

		writefile(tls12File, (serTcp.ok ? join('\n', serTcp.lines) : '') + '\n');
		writefile(tls13File, (serTcp.ok ? join('\n', serTcp.lines) : '') + '\n');
		writefile(httpFile, (serTcp.ok ? join('\n', serTcp.lines) : '') + '\n');
		writefile(quicFile, (serUdp.ok ? join('\n', serUdp.lines) : '') + '\n');

		envVars['TEST'] = 'custom';
		envVars['LIST_HTTPS_TLS12'] = tls12File;
		envVars['LIST_HTTPS_TLS13'] = tls13File;
		envVars['LIST_HTTP'] = httpFile;
		envVars['LIST_QUIC'] = quicFile;
	}

	let env = 'SCRIPT=' + shell(scripts[0]) + '\n';
	for (let key in envVars) env += key + '=' + shell(envVars[key]) + '\n';
