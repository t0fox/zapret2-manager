...
function classification() { try { let value = json(readfile(CLASSIFICATION)); if (!object(value) || (value.schema != 'zapret2-manager.z2k-integration.v1' && value.schema != 'zapret2-manager.z2k-integration.v2') || !object(value.source) || type(value.files) != 'array') return null; for (let i = 0; type(value.historicalFiles) == 'array' && i < length(value.historicalFiles); i++) push(value.files, value.historicalFiles[i]); return value; } catch (e) { return null; } }
...
function runtime_exact(item) { return item && (item.class == 'exact-managed' || item.dependencyClass == 'runtime-exact'); }
export const z2k_component_plan = function(remoteManifest) {
	...
	for (let i = 0; i < length(checked.updates); i++) {
		...
		if (runtime_exact(item)) push(exactManaged, { sourcePath: sourcePath, localName: item.localName, type: asset_type(item), sha256: checked.manifest.files_sha256[sourcePath] });
	}
