...
function validate_one(record) {
	...
	for (let payload in record.resolvedPayloads || []) if (!stat(payload.targetPath)) return { candidateId: record.candidateId, status: 'unsupported', nativeChecked: false, reason: 'payload missing at ' + payload.targetPath, cleanup: { status: 'completed' } };
	let cmd = shell_escape(NFQWS2) + ' --dry-run --qnum=30999';
	for (let option in record.compiledOptions.argv || []) cmd += ' ' + shell_escape(option);
	let p = popen(cmd + ' 2>&1', 'r'), out = p ? p.read('all') : '', rc = p ? p.close() : 127;
	return { candidateId: record.candidateId, status: rc == 0 ? 'native-adapted' : 'unsupported', nativeChecked: true, stdout: out || '', stderr: '', reason: rc == 0 ? null : trim(out) || 'nfqws2 --dry-run exited ' + rc, cleanup: { status: 'completed' } };
}
