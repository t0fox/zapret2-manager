...
import { proxy_provider_preflight, proxy_provider_check_updates } from './proxy-provider-preflight.uc';
...
if (action == 'catalog') emit(proxy_provider_catalog());
...
else if (action == 'check-updates') emit(proxy_provider_check_updates());
	...
	print('usage: proxy-provider-cli.uc catalog | status | preflight | check-updates | install <json-file> | remove <json-file> | purge <json-file>\n');
	exit(1);
}
