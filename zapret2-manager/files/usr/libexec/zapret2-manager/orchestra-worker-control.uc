...
import { orchestra_run_load, profile_set, corpus_translate, classify_attempt, control_load, proc_starttime, run, save, add_event } from './orchestra-run.uc';
...
function adapter_start(id,candidate,protocol,target,timeout) {
	let cmd="'"+ADAPTER+"' '"+id+"' '"+candidate+"' '"+protocol+"' '"+target+"' '"+timeout+"' >/dev/null 2>&1 & echo $!";
...
function done(r,candidate,protocol,attempt) { for(let x in r.results)if(x.candidateId==candidate&&x.protocol==protocol&&x.attempt==attempt)return true; return false; }
...
function finish_stop(r,id,activeAttempt) {
	...
	r.currentCandidate=null;r.currentAttempt=null;r.candidatePid=null;r.candidateStarttime=null;r.cleanup={status:'completed',checkedAt:time(),ownedChildrenStopped:true};r.phase='stopped';r.finishedAt=time();add_event(r,'cleanup','Cleanup completed; owned candidate resources stopped');save(r);return true;
}
...
export const orchestra_worker_control_run = function(id) {
	...
	let ps=profile_set(null,r.candidateMode);if(!ps||!length(ps.profiles)){r.phase='failed';r.error={code:'ESTATE',message:'no compatible trusted draft strategies for mode'};r.cleanup={status:'completed'};save(r);return false;}
	let chosen=[];
	...
	let compatible=[];for(let c2 in chosen)if(protocol_allowed(c2,r.protocols))push(compatible,c2);if(r.candidateMode=='selected'&&length(compatible)!=length(chosen)){r.phase='failed';r.error={code:'EINPUT',message:'selected candidate is incompatible with requested protocol'};r.cleanup={status:'completed'};save(r);return false;}chosen=compatible;
	r.catalogRevision=ps.revision;r.catalogHash='draft-'+ps.revision;r.totalCandidates=length(chosen);r.totalAttempts=length(chosen)*length(r.protocols)*r.repeats;r.totalCount=r.totalAttempts;r.phase='preparing';r.heartbeatAt=time();add_event(r,'preparing','Trusted candidates resolved',{count:length(chosen),compatible:true});save(r);
	...
	for(let c in chosen) for(let proto in r.protocols) for(let attempt=1;attempt<=r.repeats;attempt++) {
		...
		if(done(r,c.id,proto,attempt))continue;
		let meta=corpus_translate(c.opt),started=time();r.currentCandidate=c.id;r.currentAttempt=attempt;r.candidatePid=null;r.candidateStarttime=null;r.heartbeatAt=started;save(r);
		if(!meta.ok){let a=classify_attempt(r,c,proto,attempt,started,time(),-1,'',false,c.upstreamStrategyReference,'',meta);push(r.results,a);r.completedCount++;r.progress=r.totalCount?r.completedCount*100/r.totalCount:0;save(r);continue;}
		if(!write_list(id,c.id,proto,meta.input)){r=orchestra_run_load({runId:id});let a=classify_attempt(r,c,proto,attempt,started,time(),66,'',false,c.upstreamStrategyReference,meta.input,{ok:false,reason:'could not create custom list'});push(r.results,a);r.completedCount++;r.progress=r.totalCount?r.completedCount*100/r.totalCount:0;save(r);continue;}
		let adapter=adapter_start(id,c.id,proto,r.target,r.perAttemptTimeoutSec);if(!adapter){r=orchestra_run_load({runId:id});let a=classify_attempt(r,c,proto,attempt,started,time(),66,'',false,c.upstreamStrategyReference,meta.input,{ok:false,reason:'could not start adapter'});push(r.results,a);r.completedCount++;r.progress=r.totalCount?r.completedCount*100/r.totalCount:0;save(r);continue;}
		...
		r=orchestra_run_load({runId:id});let raw=trim(readfile(rc_file(id,c.id,proto))||''),rc=raw==''?-1:+raw,log=readfile(log_file(id,c.id,proto))||'';let a=classify_attempt(r,c,proto,attempt,started,time(),rc,log,rc==124,c.upstreamStrategyReference,meta.input,meta);push(r.results,a);r.completedCount++;r.progress=r.totalCount?r.completedCount*100/r.totalCount:0;r.candidatePid=null;r.candidateStarttime=null;r.heartbeatAt=time();add_event(r,'attempt','Candidate attempt finished',{candidateId:c.id,protocol:proto,attempt:attempt,verdict:a.verdict});save(r);
	}
