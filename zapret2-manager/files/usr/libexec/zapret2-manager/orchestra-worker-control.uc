...
export const orchestra_worker_control_run = function(id) {
	...
	for(let scope in scopes) for(let c in chosen) for(let proto in scope.protocols) for(let attempt=1;attempt<=r.repeats;attempt++) {
		...
		let meta=corpus_translate(c.opt),started=time();r.currentDomain=scope.domain;r.currentProtocol=proto;r.currentCandidate=c.id;r.currentAttempt=attempt;r.candidatePid=null;r.candidateStarttime=null;r.heartbeatAt=started;save(r);
		...
		let adapter=adapter_start(id,c.id,proto,scope.domain,r.perAttemptTimeoutSec);if(!adapter){r=orchestra_run_load({runId:id});let a=classify_attempt(r,c,proto,attempt,started,time(),66,'',false,c.upstreamStrategyReference,meta.input,{ok:false,reason:'could not start adapter'},scope.domain);push(r.results,a);r.completedCount++;r.progress=r.totalCount?r.completedCount*100/r.totalCount:0;save(r);continue;}
