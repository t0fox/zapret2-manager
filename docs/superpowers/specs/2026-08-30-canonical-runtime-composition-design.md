...
The resolver exposes three related but distinct concepts:

1. staticBase: package-owned Engine core, required Manager sidecars, and
   other explicitly approved bootstrap assets. These are the package baseline
   and are not lifecycle-managed Z2K membership.
2. productionBase: staticBase followed by the selected, exact-managed Z2K
   membership from the confirmed installed snapshot. This is the complete
   normal production closure. A selected Z2K asset is never silently replaced
   by a package copy.
3. consumerOverlay: a consumer-specific extension that is never part of the
   production closure. The scanner overlay contains scanner diagnostics only.
   Native preflight receives productionBase plus the concrete dependencies of
   the strategy under test. Install proof and postflight verify
   productionBase only.
...
Scanner-only Lua must not enter production LUAOPT.
...
~~~text
{
  ...
  membershipDigest,
  staticBase: [entry...],
  productionBase: [entry...],
  overlays: {
    scanner: [entry...],
    nativePreflight: [entry...]
  },
...
The snapshotId is a deterministic serialization/fingerprint of the authority
identity, Registry revision, manifest/classification identities, complete
selected membership, entry content identities, and canonical order. It must
not include UI state, process IDs, timestamps, or an arbitrary filesystem
listing. The membership digest is likewise deterministic and content-bound.
...
- an active exact-managed asset is absent, has a mismatched SHA/size, has a
  wrong runtime target, or is an extra lifecycle asset not authorized by the
  snapshot;
...
membership, roles, dependency providers, and order. A bounded JSON CLI wrapper,
...
~~~text
runtime-composition-cli.uc --json --consumer=production
  -> productionBase, snapshotId, order, blockingReasons

runtime-composition.uc.resolve({consumer:"native-preflight",
                                strategy:<validated strategy>})
  -> productionBase + strategyDependencies

runtime-composition-cli.uc --json --consumer=scanner
  -> productionBase + overlays.scanner

runtime-composition-cli.uc --json --consumer=install-proof
  -> productionBase + snapshotId

runtime-composition-cli.uc --json --consumer=postflight
  -> productionBase + snapshotId + process-match requirements
~~~

The shell runtime synchronizer invokes the bounded CLI once, validates the
snapshot and result state, and builds its --lua-init invocation directly from
the returned ordered productionBase. It does not contain a Lua array, path
list, fallback sort, or existence-based append logic.

Native preflight imports the same UCode resolver module and receives the same
ordered productionBase; it then asks that resolver to resolve the concrete
strategy dependency closure. It does not reconstruct the order from a
manifest or package directory.
...
membership or production LUAOPT.
...
Prepare persists the complete resolver identity needed for Apply:

- snapshotId;
- Registry revision;
- installed authority and receipt identity;
- source commit/manifest/classification identities;
- membership digest;
- ordered entry identities and content SHA-256 values;
- target/plan-token identity already required by the update transaction.

Immediately before Apply, the system resolves the current composition again
and compares the persisted identity and all content-bound fields. If the
Registry revision, receipt, target membership, classification, source
identity, content SHA, or order differs, Apply returns ESTALE/equivalent and
performs no activation. A presentation cache or a stale generated closure
cannot satisfy this compare-and-swap gate.

After a successful Registry commit and activation, postflight must verify the
new resolver snapshot. A mutation cannot report success based only on files
written or Registry rows updated.
...
productionBase selected for runtime. It must resolve, at minimum:

- every referenced Lua function and its provider entry;
- explicit lua-init references;
- Lua-init dependency order;
- referenced blobs, hostlists, ipsets, and other list assets;
...
The provider inventory is derived from the selected snapshot entries and their
declared capabilities, not from “a file exists under /opt/zapret2”. An absent
provider, a provider removed by the target release, an unselected file, or a
mismatched SHA blocks validation. A Strategy that references a function whose
provider was removed must fail even if a package or stale filesystem copy
happens to exist.

The native command line used for the dry-run is generated from the resolver
ordered result. Preflight must not add scanner-only assets and must not
silently add a package file to make a missing dependency pass.
...
normalized, ordered --lua-init entries to the resolver's expected
productionBase. It must detect and fail on:
...
The process evidence must be tied to the activation generation/snapshot, not
only to a path string. The activation boundary records a content-bound
runtime generation derived from the snapshot and active runtime configuration;
postflight verifies that the observed process/config generation corresponds to
the new snapshot. A stale process with a seemingly valid subset of paths is
not success.
...
productionBase. The scanner may receive a separate diagnostic overlay for
scanner-only instrumentation or helper Lua. The overlay:

- is explicit and ordered by the resolver;
- is not part of productionBase;
- is not written to the production Registry membership;
- is not emitted into production LUAOPT;
- is not used to authorize a mutation;
- must not hide a missing production dependency.

The comparison test is structural: scanner base entry IDs/order/content
identities must equal production base entry IDs/order/content identities, while
scanner overlay entries must be absent from production.
...
- unresolved Strategy provider/dependency;
...
| C | Scanner and production resolve the same base | Base IDs/order/content match; scanner-only overlay is not production |
| D | Strategy references a function whose provider was removed | Native validation fails |
...
- the resolver emits A(new) and C for production, native preflight, scanner
  base, install proof, and postflight without consumer source-list changes;
...
- Scanner consumes base plus explicit overlay.
...
- [x] Production base and consumer-specific overlays are separated.
- [x] Native preflight uses the same base plus concrete Strategy dependencies.
- [x] Scanner uses the same base plus scanner-only diagnostic overlay.
- [x] One resolver owns membership, provider mapping, and order.
...
- [x] Stale running argv, removed Lua, missing selected Lua, wrong order, and
      old generation are postflight failures.
- [x] Strategy function, lua-init, blob, hostlist, and ipset dependencies are
      snapshot-bound.
- [x] Future classified additions flow to all consumers without consumer list
      edits; unknown additions remain review-required.
- [x] Test matrix contains cases A-F and the future-release behavior.
