...
Вспомогательные файлы, создаваемые для digest-проверки Profile, являются только временными: после вычисления hash они удаляются через `fs.unlink` и не входят в persistent Strategy state.
