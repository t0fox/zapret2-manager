...
Для OpenWrt target acceptance probe executor также сохраняет совместимость с
версией ucode, установленной на роутере: выражения с non-capturing regex
groups `(?:...)` не используются. Это runtime compatibility detail, а не
новый Scanner owner; временный probe lifecycle и cleanup остаются прежними.
