...
| DNS routing | Per-domain resolver rules and quick presets | Add rule, select DNS server, delete, apply, toast result | Existing DNS/provider/service-DNS writers; canonical `dns_product_*` facade | DNS page is provider/service oriented and not donor-shaped | PARTIAL | Browser proof remains pending; preserve canonical facade and single DNS writer | `web/js/pages/dns_routing.js`, `web/js/components/toast.js` |
...
| Telegram Proxy | Install/uninstall, engine detection, status polling | Confirm install/remove, poll progress, start/stop/restart | `proxy_*`, registered provider RPC, canonical Go/Rust runtime model | Proxy page exists with Z2M-specific Go↔Rust adapter | PARTIAL | Browser proof remains pending; keep provider lifecycle delegated to the existing owner | `web/js/pages/tgproxy.js`, `web/js/components/setup_ui.js`, `web/js/components/confirm.js` |
