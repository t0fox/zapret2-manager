...
| tg.versions | telegram / telegram-tunnel/versions | Installed/package/latest/available metadata | status + bounded proxy_provider_versions/check_updates | ADAPTED | Go v1.4.1 direct binary from the canonical GitHub release, release metadata/notes and compatibility fields; target PASS; browser recheck pending auth |
