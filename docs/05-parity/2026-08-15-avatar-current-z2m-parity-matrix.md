...
| tg.versions | telegram / telegram-tunnel/versions | Installed/package/latest/available metadata | status + bounded proxy_provider_versions/check_updates | ADAPTED | release metadata/notes, compatibility fields, target PASS; browser recheck pending auth |
| tg.version_selector | telegram / telegram-tunnel/versions | Select available version | proxy_provider_versions + provider/source/version/checkToken install input | ADAPTED | clean version labels; unavailable releases carry exact reason; browser recheck pending auth |
