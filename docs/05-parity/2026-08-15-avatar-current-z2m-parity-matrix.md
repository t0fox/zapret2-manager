...
| DNS routing | Per-domain resolver rules and quick presets | Add rule, select DNS server, delete, apply, toast result | Existing DNS/provider/service-DNS writers; canonical `dns_product_*` facade in progress | DNS page is provider/service oriented and not donor-shaped | PARTIAL | Use donor rule/preset behavior over canonical facade; add read/preview/apply/rollback tests | `web/js/pages/dns_routing.js`, `web/js/components/toast.js` |
| DNS providers | Provider catalog and diagnosis | Inspect providers, diagnose, select/apply | `dnsprov_*`, `dns_select_provider`, global/override DNS writers | DNS page exposes partial provider and dependency-gate states | PARTIAL | Preserve native ownership while matching donor cards, errors and disabled states | `web/js/pages/dns_routing.js`, `web/js/pages/diagnostics.js` |
| Telegram Proxy | Install/uninstall, engine detection, status polling | Confirm install/remove, poll progress, start/stop/restart | `proxy_*`, provider RPC, Go/Rust runtime evidence | Proxy page exists with Z2M-specific Go↔Rust adapter | PARTIAL | Transplant donor lifecycle/config/connection interactions onto canonical Z2M adapter | `web/js/pages/tgproxy.js`, `web/js/components/setup_ui.js`, `web/js/components/confirm.js` |
| Telegram Proxy config | TG-WS config, tunnels, routes, secret rotation | Edit/save, add/remove route, rotate secret, connect-info copy | `proxy_config_*`, `proxy_secret_rotate`, runtime adapter | Existing config surface is narrower than donor | PARTIAL | Match donor forms, validation, tunnel rows, clipboard and destructive confirmations | `web/js/pages/tgproxy.js`, `web/js/components/toast.js` |
...
interactions over the existing Z2M proxy/provider contract. These are adapted
surfaces, not `EXACT` rows: the target Browser gate still reports missing
engine/provider RPC objects, so DNS/TG rows remain `PARTIAL` pending backend
registration and full Browser flow evidence.
