...
Основа пакетов, LuCI, backend, state model и native helpers существует в текущем репозитории. `Strategy` реализована и развивается. **Сканер (`Scanner`)** содержит существенный объём кода, но остаётся прототипом в активной разработке. M5 добавляет отдельные `BlockCheck`, `Block Detector`, `BlockCheck2` и `BlockCheckW` verticals; target-router evidence проверяется отдельно.
