---
id: public-guides-index
title: "Практические руководства"
type: guide-index
status: current
authority: user-guide
updated: 2026-08-22
publish: true
tags: [guides, howto]
---

# Практические руководства

- [Как выбрать и применить Strategy](./choose-strategy.md)
- [Как использовать Z2K Detect](./use-scanner.md)
- [Как добавить домен вручную](./add-domain.md)
- [Как настроить DNS](./check-dns.md)
- [Как установить Telegram Proxy](./install-telegram-proxy.md)
- [Как сделать резервную копию](./make-backup.md)
- [Что проверить после обновления](./after-update.md)
- [Как собрать диагностику](./collect-diagnostics.md)
